...
resource "aws_nat_gateway" "eks_vpc_nw" {
  allocation_id = aws_eip.eks_eip.id
  subnet_id     = aws_subnet.public[0].id
}

resource "aws_route" "eks_vpc_private_nat_route" {
  route_table_id         = aws_vpc.eks_vpc.default_route_table_id
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id          = aws_nat_gateway.eks_vpc_nw.id
  # depends_on             = [aws_vpc.eks_vpc.default_route_table_id]
}
